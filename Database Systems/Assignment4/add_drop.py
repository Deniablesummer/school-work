...
with open(input_filename) as f:
	for row in csv.reader(f):
		...
		if len(row) < 4:
			...
			break
		action, id, name, code, term = row[0:5]
		
		try:
			if action == 'ADD':		
				cursor.execute("insert into students values(%s, %s) on conflict do nothing;", (id, name));
				cursor.execute("insert into enrollment values(%s, %s, %s);", (code, term, id));
			elif action == 'DROP':
				cursor.execute("delete from enrollment where course_code = %s and term_code = %s and id = %s;", (code, term, id));
			else:
				conn.rollback()
