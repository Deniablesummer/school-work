 ...
 *
 * Author: James Ryan
 * Version: 2 - Software Optimized Implementation
 *
...
int main() {
    ...
    uint8_t *y[horz];
    unsigned int cb_avg[horz>>1];
    unsigned int cr_avg[horz>>1];
    unsigned int k;
	...
	// Average Cb and Cr values for 2 by 2 pixels
        for (j=0; j<horz; j+=2) {
            k = j>>1;
            cb_avg[k] = avg(twoRows[0][j].byte2, twoRows[0][j+1].byte2, twoRows[1][j].byte2, twoRows[1][j+1].byte2);
            cr_avg[k] = avg(twoRows[0][j].byte3, twoRows[0][j+1].byte3, twoRows[1][j].byte3, twoRows[1][j+1].byte3);
        }

        // End of RGB to YCC - now convert back to RGB and write to file
        for (j=0; j < horz; j+=2) { //row1
            k = j>>1;
            x = convertYCCtoRGB(twoRows[0][j].byte1, cb_avg[k], cr_avg[k]);
            fwrite(&x[0], 1, 1, outputfile);
            fwrite(&x[1], 1, 1, outputfile);
            fwrite(&x[2], 1, 1, outputfile);
            x = convertYCCtoRGB(twoRows[0][j+1].byte1, cb_avg[k], cr_avg[k]);
            fwrite(&x[0], 1, 1, outputfile);
            fwrite(&x[1], 1, 1, outputfile);
            fwrite(&x[2], 1, 1, outputfile);
        }
        for (j=0; j < horz; j+=2) { //row2
            k = j>>1;
            x = convertYCCtoRGB(twoRows[1][j].byte1, cb_avg[k], cr_avg[k]);
            fwrite(&x[0], 1, 1, outputfile);
            fwrite(&x[1], 1, 1, outputfile);
            fwrite(&x[2], 1, 1, outputfile);
            x = convertYCCtoRGB(twoRows[1][j+1].byte1, cb_avg[k], cr_avg[k]);
            fwrite(&x[0], 1, 1, outputfile);
            fwrite(&x[1], 1, 1, outputfile);
            fwrite(&x[2], 1, 1, outputfile);
        }
 ...
 *
 * Improved version using bit shifts. Shifts found in resource (1)
 *
 ...
 */
uint8_t *convertYCCtoRGB(uint8_t y, uint8_t cb, uint8_t cr) {
    uint8_t *RGB = malloc(sizeof(u_int8_t)*3);
