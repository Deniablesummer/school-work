<template>
  <div class="buy-stock">
    <h1>Set Buy Trigger</h1>
    <SetTriggerForm v-on:trigger-set="showSetAmountForm"></SetTriggerForm>
    <SetAmountForm v-if="this.buyTriggerPrice" v-on:user-set-amount="setBuyAmount"></SetAmountForm>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setBuyAmount(dollarAmount) {
      ...
      let socketMessage = '[1] SET_BUY_AMOUNT '+user+' '+this.stockSymbol+' '+dollarAmount
      console.log("sending "+socketMessage)
      this.$socket.send(socketMessage)
      this.$socket.onmessage = (data) => {
        alert(data.data)
        socketMessage = '[1] SET_BUY_TRIGGER '+user+' '+this.stockSymbol+' '+this.buyTriggerPrice
        console.log("sending "+socketMessage)
        this.$socket.send(socketMessage)
        this.$socket.onmessage = (data) => {
            alert(data.data)
